# Eval-Ops M10 Â· ç”Ÿäº§æ ˆå¤‡ä»½ï¼šPostgreSQL dump + uploads å‘½åå?# å‰ç½®ï¼šdocker compose -f docker-compose.yml -f docker-compose.prod.yml up -d
# ç”¨æ³•ï¼?\scripts\backup-prod.ps1
...
if (-not $pgRunning) {
    Write-Fail "postgres not running â€?start prod stack first (see docs/DEPLOY.md Â§3.3)"
}

if (-not (Test-DockerVolume $UploadsVolume)) {
    Write-Fail "uploads volume missing: $UploadsVolume â€?use prod compose (docker-compose.prod.yml)"
}
...
if ($dumpSize -lt 100) { Write-Fail "dump file too small ($dumpSize bytes) â€?check postgres logs" }

Write-Step "uploads volume â†?$uploadsTar"
...
Write-Pass "backup complete â†?$dest"
