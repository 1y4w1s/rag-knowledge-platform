...
(async () => {
  ...
  // 登录 demo_member（personal workspace，111 是 0 文档库）
  ...
  await page.screenshot({
    path: path.join(OUT, "verify_p1_2_kb_list_member.png"),
    clip: { x: 0, y: 0, width: 1440, height: 800 },
  });
  console.log("✓ member /knowledge-bases → verify_p1_2_kb_list_member.png");
  await browser.close();
})().catch((e) => {
  console.error(e);
  process.exit(1);
