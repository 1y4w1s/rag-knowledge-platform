"""019 — backfill G1 chat_messages into default chat_threads

Revision ID: 019
Revises: 018
Create Date: 2026-07-09

G2-0.2: H2-2-A — one 「历史对话」 thread per scope group; thread_id stays
nullable until G2-0.3 persistence writes threads on new messages.
"""
...
def _backfill_chat_threads() -> None:
    conn = op.get_bind()
...
def upgrade() -> None:
    _backfill_chat_threads()


def downgrade() -> None:
    op.execute(sa.text("UPDATE chat_messages SET thread_id = NULL"))
