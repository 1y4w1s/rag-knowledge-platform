...
async def test_org_admin_list_audit_logs_200(
    ...
    uploads = [item for item in body["items"] if item["action"] == "document.upload"]
    assert uploads
    assert all(item["actor_user_id"] == admin_user["id"] for item in uploads)
    assert all(item.get("actor_email") == admin_user["email"] for item in uploads)
    assert all(item.get("kb_name") == "审计库" for item in uploads)
