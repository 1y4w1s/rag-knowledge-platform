...
async def main():
    ...
    async with SessionLocal() as db:
        for i, case in enumerate(cases):
            e = case.get("expect", {})
            cc = e.get("content_contains", "").lower()
            sp = e.get("section_title", "").lower()
            hp = e.get("heading_path_contains", "").lower()
            ch = await retrieve_chunks(db, kb_id=kb_id, query=case["query"], top_k=3)
            hit = False
            if ch:
                for ck in ch[:3]:
                    c = (ck.content or "").lower()
                    s = (ck.heading_path or ck.section_title or "").lower()
                    ok = True
                    if cc and cc not in c:
                        ok = False
                    if sp and sp not in s:
                        ok = False
                    if hp and hp not in s:
                        ok = False
                    if ok:
                        hit = True
                        break
