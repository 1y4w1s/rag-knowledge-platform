...
export function RagProofCard({ hitRate, evaluatedAt, note }: RagProofCardProps) {
  ...
  const dateStr = evaluatedAt ? `评估于 ${evaluatedAt}` : "";
  ...
  return (
    <div className="rounded-2xl border border-[var(--line2)] bg-[var(--surf)] p-5 shadow-[var(--top-hi),var(--card-shadow)] transition-colors hover:border-[var(--line)]">
      <div className="mb-4 text-center text-[13px] text-[var(--mut)] tracking-wide">
        检索可证明性 · Golden Hit@3
      </div>
      <div className="text-center">
        <div className="font-[var(--serif)] text-[46px] font-semibold leading-none text-[var(--ok)] tabular-nums">
          {pct}
        </div>
