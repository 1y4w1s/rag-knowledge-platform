...
export function KnowledgeBaseCard({
  ...
  return (
    <article className="kb-card relative rounded-[14px] border border-[var(--line2)] bg-[color-mix(in_srgb,var(--surf)_82%,transparent)] p-4 transition-colors hover:border-[color-mix(in_srgb,var(--action)_28%,var(--line2))] hover:bg-[var(--surf)]">
      ...
      <div className="mt-3.5 flex flex-wrap items-center gap-2">
        <Link
          ...
          className="kb-card-enter inline-flex items-center gap-1 rounded-[8px] py-1.5 text-[0.75rem] font-bold text-[var(--action)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring-action)]"
        >
          进入
        ...
        {canEdit && onEdit ? (
          <Button
            ...
            size="sm"
            onClick={() => onEdit(kb)}
            className="rounded-[8px] text-[0.75rem] font-medium text-[var(--mut)] hover:bg-[var(--surf2)] hover:text-foreground"
          >
            编辑
          </Button>
        ) : onMemberWriteBlocked ? (
        ...
        {canDelete ? (
          <Button
            ...
            disabled={deleting}
            onClick={() => onDelete(kb)}
            className="rounded-[8px] text-[0.75rem] font-medium text-[var(--mut)] hover:bg-[var(--bad)]/10 hover:text-[var(--bad)]"
          >
            {deleting ? "删除中…" : "删除"}
          </Button>
        ) : onMemberWriteBlocked ? (
