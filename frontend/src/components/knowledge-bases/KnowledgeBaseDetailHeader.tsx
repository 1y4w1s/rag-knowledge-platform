...
export function KnowledgeBaseDetailHeader({
  ...
  return (
    <header className="mb-5 flex flex-wrap items-start justify-between gap-4 pb-2">
      <div className="min-w-0 flex-1">
        ...
        <h1 className="mt-2 font-serif text-[1.5rem] font-semibold leading-tight text-foreground">
          {kb.name}
        </h1>
        {kb.description ? (
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2">
        ...
        {chatAllowed ? (
          <Link
            ...
            className="inline-flex items-center rounded-[8px] px-1 py-1.5 text-[0.75rem] font-bold text-[var(--action)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring-action)]"
          >
            开始对话 →
          </Link>
        ) : (
          <Button
            ...
            size="sm"
            variant="ghost"
            className="cursor-not-allowed px-1 font-bold text-[var(--action)] opacity-60"
