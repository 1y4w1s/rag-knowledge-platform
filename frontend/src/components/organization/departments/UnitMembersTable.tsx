...
export function UnitMembersTable({
  ...
  if (members.length === 0) {
    return (
      <p className="rounded-[10px] border border-dashed border-[var(--line2)] bg-[rgba(245,242,237,0.45)] px-4 py-8 text-center text-sm text-muted">
        暂无成员。从公司花名册添加成员到此部门。
      </p>
    );
  }
        ...
        {members.map((member) => {
          ...
          return (
            <tr key={member.user_id}>
              ...
              <td>
                <span className="doc-badge doc-badge-wait">
