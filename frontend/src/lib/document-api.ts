  ...
  return new Promise<Document[]>((resolve, reject) => {
    ...
    xhr.addEventListener("load", () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          const body = JSON.parse(xhr.responseText) as { documents: Document[] };
    ...
    xhr.open("POST", `${API_BASE}/knowledge-bases/${kbId}/documents`);
    // Copy auth token
    const token = getAccessToken();
    if (token) {
...
/**
 * 构建 PDF / 文本预览 iframe 直链。
 * 不走 blob URL（Chrome 对 blob: 协议默认 X-Frame-Options: DENY，iframe 嵌入会被拒绝）。
 * 走同源相对路径（Vite dev proxy 到后端），用 query token 携带 JWT 让后端鉴权。
