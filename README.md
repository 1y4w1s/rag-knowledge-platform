...
基础链路是完整且默认的：解析 → 结构化切片 → Hybrid 检索 → 引用溯源生成，每一步都可直接使用。更高级的手段不是没有，而是按评测结果默认关闭——rerank、多查询、HyDE、图谱召回仅在信号明确时触发，不为没有验证的收益增加默认开销。

Agent 侧保持受控：确定性规划 + 有限 LLM 规划、11 个工具、写审批、分层记忆。它不是完全自主的多智能体，而是把能力放进审计与审批边界内。

企业工程化是闭环：离线评测门禁、审计、权限、脱敏、SLO、内网 HTTP 部署都已落地。

明确不做：多模态、公网 SaaS、支付计费。这是产品边界，不是能力缺口。
