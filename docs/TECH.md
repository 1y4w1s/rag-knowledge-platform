...
| 状态 | `completed` / `failed` 可预览；处理中 → 409 |
| Content-Type | `pdf` → `application/pdf`；`txt`/`md` → `text/plain`；`docx` 抽文本成功 → 纯文本，否则 OOXML；`png`/`jpg`/`jpeg` → 图片 inline；`xlsx`/`pptx` → 附件下载 |
| 响应 | 磁盘 `storage_path` 流式返回；前端按类型内嵌或下载 |

**大白话**：把库里存的原文件读出来给你看——PDF/图片浏览器直接开，文本当文本显示，表格类先下载。
